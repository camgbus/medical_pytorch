...
class UKF2(SegmentationDataset):
    ...
    def __init__(self, hold_out_ixs=None,resized=True):
        ...
        if resized:
            dataset_path = os.path.join(storage_data_path, global_name+'_resized')
        else:
        ...
        if not os.path.isdir(dataset_path):
            _extract_images(original_data_path, dataset_path,resized)
...
def _extract_images(source_path, target_path,resized):
