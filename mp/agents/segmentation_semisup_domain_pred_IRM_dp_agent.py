...
class SegmentationSemisupDomainPredictionIRMAgent(SegmentationSemisupDomainPredictionAgent):
    ...
    with an IRM loss for the domain predictor.
    IRM training schedules are completely ignored.
    """
    ...
    def perform_stage2_training_epoch(self, optimizer_model,
        ...
        for data_list_seg, data_list_dp in zip(zip_longest_with_cycle(*train_dataloaders_seg),
            ...
            dl_cnt = len(train_dataloaders_dp)  # How many training environments
            domain_pred_losses = []
            domain_pred_penalties = []
            domain_targets = []
            # For each dataloader
            for idx, data_dp in enumerate(data_list_dp):
                # Get data
                inputs, targets = self.get_inputs_targets(data_dp)
                feature = self.model.get_features_from_encoder(inputs)

                domain_pred = softmax(self.model.get_domain_prediction_from_features(feature))
                domain_target = torch.zeros((inputs.shape[0], dl_cnt), dtype=targets.dtype).to(self.device)
                domain_target[:, idx] = 1
                domain_targets.append(domain_target)
                domain_pred_output = softmax(domain_pred)

                domain_pred_losses.append(irm_loss_f_domain_pred.erm(domain_pred_output, domain_target))
                domain_pred_penalties.append(irm_loss_f_domain_pred(domain_pred_output, domain_target))
            ...
            loss_encoder = beta * loss_f_encoder(domain_pred, torch.cat(domain_targets, dim=0))
    ...
    def train_with_early_stopping(self, results, optimizers, losses,
        ...
        return super().train_with_early_stopping(results, optimizers, losses, train_dataloaders_seg,
                                                 train_dataloaders_dp, train_dataset_names,
                                                 early_stopping, init_epoch=init_epoch,
                                                 run_loss_print_interval=run_loss_print_interval,
                                                 eval_datasets_seg=eval_datasets_seg, eval_datasets_dp=eval_datasets_dp,
