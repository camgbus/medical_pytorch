...
from mp.data.datasets.ds_mr_lung_decathlon_reg import DecathlonLungAlexNet
...
from mp.models.regression.linear_regression import AlexNet
from mp.eval.losses.losses_regression import LossMAE, LossMSE, LossHuber, LossCEL
from mp.agents.regression_agent import AlexNetAgent
...
# 3. Define data
data = Data()
data.add_dataset(DecathlonLungAlexNet(augmented=True, img_size=input_shape,
...
for run_ix in range(config['nr_runs']):
    ...
    losses_train, losses_cum_train = agent.\
    ...
    losses_cum_test = agent.test(loss_f, dl)
