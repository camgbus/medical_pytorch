...
configs = [
    {'experiment_name': 'harp(24,6)_dryad_hybrid_aug_erm', 'device': 'cuda:0',
...
for config in configs:
    ...
    for run_ix in range(config.get("start", 0), config['nr_runs']):
        ...
        optimizer_encoder = optim.Adam(model.encoder_parameters(), lr=5e-4)
        optimizers = [optimizer_stage1, optimizer_model, optimizer_domain_predictor, optimizer_encoder]

        # 11. Train model
        results = Result(name='training_trajectory')
        agent = SegmentationSemisupDomainPredictionAgent(model=model, label_names=label_names, device=device,
