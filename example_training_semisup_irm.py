...
configs = [
    {'experiment_name': 'harp(24,6)_dryad_hybrid_aug_nodompred_erm', 'device': 'cuda:0',
     'nr_runs': 5, 'cross_validation': True, 'val_ratio': 0.1, 'test_ratio': 0.3,
     'input_shape': (1, 48, 64, 64), 'resize': False, 'augmentation': 'hybrid',
     'class_weights': (0., 1.), 'lr': 1.5e-4, 'batch_sizes': [4, 8],
     "loss": "erm", "penalty_weight": 1e5,
     "eval_interval": 20,
     "train_ds_names": (harp.name, dryad.name),
     "limited_labels": {harp.name: (24, 6)},
     },
    {'experiment_name': 'harp(24,6)_dryad_hybrid_aug_nodompred_vrex', 'device': 'cuda:0',
     'nr_runs': 5, 'cross_validation': True, 'val_ratio': 0.1, 'test_ratio': 0.3,
     'input_shape': (1, 48, 64, 64), 'resize': False, 'augmentation': 'hybrid',
     'class_weights': (0., 1.), 'lr': 1.5e-4, 'batch_sizes': [4, 8],
     "loss": "vrex", "penalty_weight": 1e5,
     "eval_interval": 20,
     "train_ds_names": (harp.name, dryad.name),
     "limited_labels": {harp.name: (24, 6)},
     },
    {'experiment_name': 'harp(24,6)_dryad_hybrid_aug_nodompred_irmv1', 'device': 'cuda:0',
     'nr_runs': 5, 'cross_validation': True, 'val_ratio': 0.1, 'test_ratio': 0.3,
     'input_shape': (1, 48, 64, 64), 'resize': False, 'augmentation': 'hybrid',
     'class_weights': (0., 1.), 'lr': 1.5e-4, 'batch_sizes': [4, 8],
     "loss": "irmv1", "penalty_weight": 1e5,
     "eval_interval": 20,
     "train_ds_names": (harp.name, dryad.name),
     "limited_labels": {harp.name: (24, 6)},
     },
...
for config in configs:
    ...
    for run_ix in range(config.get("start", 0), config['nr_runs']):
        ...
        epochs = agent.train_with_early_stopping(results, optimizer, irm_loss,
                                                 train_dataloaders=dls,
